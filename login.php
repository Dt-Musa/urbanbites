...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($username === '' || $password === '') {
      ...
      $stmt = mysqli_prepare($conn, 'SELECT id, username, email, password FROM users WHERE username = ? OR email = ? LIMIT 1');
      ...
      if ($user && !$passwordMatches && $password === $user['password']) {
        ...
        mysqli_stmt_bind_param($updateStmt, 'si', $rehash, $user['id']);
...
<head>
  ...
  <style>
    ...
    .session-note { margin-bottom: 20px; padding: 14px 16px; border-radius: 10px; background: rgba(200,151,58,0.1); color: var(--dark); font-size: 0.92rem; }
    .session-note a { color: var(--gold); font-weight: 700; text-decoration: none; }
  ...
  <div class="auth-container">
    <div class="auth-card">
      <div class="auth-header">
        ...
        <h1>Login</h1>
        <p>Sign in to your Urban Bites account</p>
      </div>

      <?php if ($sessionActive): ?>
        <div class="session-note">
          You are already signed in. <a href="login.php?reset=1">Switch account</a>
        </div>
      <?php endif; ?>
